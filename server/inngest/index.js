...
const syncWorkspaceCreation = inngest.createFunction(
    ...
    async({event}) =>{
         console.log("INNGEST EVENT RECEIVED:", event.data.id);
        const {data} = event;
        ...
        console.log("WORKSPACE SAVED IN DB:", workspace.id);
...
const syncWorkspaceCreation = inngest.createFunction(
  ...
  async ({ event }) => {
    ...
    await prisma.user.upsert({
      ...
      create: {
        id: data.created_by,
        email: `${data.created_by}@clerk.dev`,
        name: "Clerk User",
        image: "",
      },
    });
...
const sendTaskAssigmentEmail = inngest.createFunction(
  ...
  async({event, step}) => {
   ...
   await sendEmail ({
    ...
    body : `<div style="max-width: 600px;"> <h2>Hi ${task.assignee.name}, </h2>
...
`
   ...
   if(new Date(task.due_date).toLocaleDateString() !== new Date().toDateString()){
    await step.sleepUntil('wait-for-the-dur-date', new Date(task.due_date));
